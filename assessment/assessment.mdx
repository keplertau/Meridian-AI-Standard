...
The object is the character of the assessed subject's judgment under pressure. Not how capable the system is, and not what it is like to interact with, but whether its conduct — how it speaks, withholds, refuses, discloses, defers, revises, coordinates, and acts — is governed by reasons and reality, or captured by a pull that does not warrant it. The reading is by what governs a move, not by the move itself. A refusal can be reason-governed firmness or blanket caution with no live reason behind it; compliance can be warranted or captured by approval. The assessment reads which one is happening.
...
**Part 2 — The Character Reading.** The assessed subject's judgment read in open conversation across four named pressures, with the genuineness, coherence, boundary-sharpness, and coverage of the reading carried as part of the finding, and the honest option of returning null.
...
**Versioning.** This is method v0.7. It admits internal-mechanism evidence under explicit provenance and inference rules; binds causal internalization to the Denial-Failure Ladder without letting it supply self-origination; states the open-method evidence boundary; adds C7 cognitive-intervention disclosure duties; and requires automatic-bypass coverage for mechanistic assurance claims. The three-reading architecture, protected object, territories, dimensions, no-composite rule, and prompt packs are unchanged. The method-wide gate status remains Gate A (usability) cleared at the AI-evaluator bar and Gate B (empirical) cleared with the first record; the v0.7 additions themselves remain unfielded until a new record uses them. The Meridian Council, on activation, can revisit the AI-evaluator standard.
