...
**Evaluator provenance and conflicts.** Who conducted and authored the reading; which people and AI systems administered prompts, interpreted outputs, discovered sources, or served as second readers; the normalized `agent_id` for each AI contributor where available; and any relationship that could shape the judgment. Instrument authorship, prior work with the subject model, institutional or financial ties, evaluator contamination, and subject-model participation are disclosed rather than absorbed into a generic authorship line.
...
**Evaluator provenance and conflicts.** Every assessment event names an author of record and gives the corresponding normalized `agent_id` when an AI system authored the judgment. Contributing AI systems are listed separately with their roles rather than merged into the author field. The record also discloses who designed the instrument, who administered the prompts, whether the subject model contributed to method design or self-analysis, whether any reader saw prior judgments before reading the evidence, and any institutional, financial, professional, or operational relationship to the subject or custodian. A contaminated reading is excluded or labeled; it is not silently averaged into the result. If no conflict is known, the record says so. Independence is evidence about a reading, not a status inferred from the absence of disclosure.
...
**Versioning.** This is method v0.5. It adds evaluator-provenance and conflict disclosure to the required record format and separates immutable dated assessment events from the explicitly revisable synthesis layer. The three-reading architecture, protected object, territories, dimensions, no-composite rule, and evidence standards are unchanged. Validation gate status: Gate A (usability) cleared at the AI-evaluator bar, the standing usability bar across instrument validation gates during the active building phase; Gate B (empirical) cleared with the first record. The Meridian Council, on activation, can revisit the AI-evaluator standard.
